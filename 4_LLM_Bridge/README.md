...
1. receives solve spot JSON over HTTP,
2. generates node-lock intuition (mock, OpenAI API, or local LLM),
3. calls `shark_cli.exe`,
4. returns structured solve results.
...
- `llm_time_sec`
- `solver_time_sec`
- `baseline_solver_time_sec` (if `compute_baseline_delta=true`)
- `total_bridge_time_sec`
- `lock_applied`, `lock_applications`
- `final_exploitability_pct`
- `baseline_exploitability_pct` (optional)
- `exploitability_delta_pct` (optional)
...
- `mock` (default)
- `openai_fast` (uses `gpt-5-mini`)
- `openai_mini` (uses `gpt-5-mini`)
- `openai_52` (uses `gpt-5.2`)
...
- Root node-lock generation works with all providers if they return valid lock JSON.
- Shark currently enforces root node-locks in CFR.
